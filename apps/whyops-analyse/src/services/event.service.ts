...
import { normalizeEventContent } from '@whyops/shared/lib/event-normalizer';
...
export class EventService {
  ...
  static async processEvent(data: EventData): Promise<EventProcessResult> {
    ...
    return traceQueue.getQueue(data.traceId).add(async () => {
      ...
      const normalizedContent = normalizeEventContent(data.eventType, data.content, data.metadata);

      // Extract typed hot fields from metadata for efficient aggregation queries
      ...
      const typedFields = {
        ...
        cacheWriteTokens: usage.cacheWrite5mTokens ?? usage.cacheCreationTokens ?? usage.cacheWriteTokens ?? null,
        latencyMs: data.metadata?.latencyMs ?? null,
        finishReason: (normalizedContent as any)?.finishReason ?? null,
      };
      ...
      // Invalidate compressed payload cache so the next thread detail read rebuilds it
      await Trace.update(
        { eventsPayload: null, eventsPayloadAt: null },
  ...
  private static async createToolCallEvents(data: {
    ...
    const normalizedRequest = normalizeEventContent('tool_call_request', data.content, data.metadata);
    const normalizedResponse = normalizeEventContent('tool_call_response', { toolCalls, toolResults }, data.metadata);
